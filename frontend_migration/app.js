...
function renderSidebar(activeView) {
  ...
  header.appendChild(el('img', { src: '/static/lens_image.png', alt: 'RepoLens' }));
  header.appendChild(el('span', { class: 'sidebar-header-title' }, 'RepoLens'));
  ...
  const searchBtn = el('button', {
    ...
    el('span', {}, 'Search'),
    el('kbd', { style: { marginLeft: 'auto', fontSize: '11px', padding: '1px 4px', background: 'var(--bg-input)', border: '1px solid var(--border-default)', borderRadius: '3px', color: 'var(--text-tertiary)' } }, '⌘K'),
  );
...
const PIPELINE_STAGES = [
  { id: 'clone', label: 'Repository opened' },
  { id: 'tree', label: 'Parsing Tree' },
  { id: 'nodes', label: 'Extracting Nodes & Edges' },
  { id: 'summary', label: 'Generating Summaries & Index' },
...
async function startPipeline() {
  ...
  try {
    pipelineData.stage_index = 1; pipelineData.stage = 'tree'; updatePipeline(pipelineData);
    log('Starting Tree Sitter parsing...');
    await api('/tree', { method: 'POST' });
    
    pipelineData.stage_index = 2; pipelineData.stage = 'nodes'; updatePipeline(pipelineData);
    log('Extracting Nodes...');
    await api('/nodes', { method: 'POST' });
    log('Extracting Edges...');
    await api('/edges', { method: 'POST' });
    
    pipelineData.stage_index = 3; pipelineData.stage = 'summary'; updatePipeline(pipelineData);
  ...
  if (isExpanded) {
    ...
    if (isFile && node.nodes) {
      for (const n of node.nodes) {
        const nodeItem = el('div', { class: 'tree-node' });
        const isNodeExpanded = state.expandedNodes.has(n.id);
        
        const ni = el('div', { class: 'tree-item', onClick: (e) => { e.stopPropagation(); toggleNode(n.id); } });
        const toggle = el('span', { class: `tree-toggle ${isNodeExpanded ? 'expanded' : ''}`, html: Icons.chevronRight });
        ni.appendChild(toggle);
        
        const typeIcon = n.node_type?.includes('class') ? '◆' : n.node_type?.includes('function') || n.node_type?.includes('method') ? 'ƒ' : '•';
        ni.appendChild(el('span', { class: 'tree-item-icon', style: { color: 'var(--accent)' }, html: typeIcon }));
        
        const nc = el('div', { class: 'tree-item-content' });
        nc.appendChild(el('div', { class: 'tree-item-name' }, n.title || n.id));
        
        if (n.summary && !isNodeExpanded) {
           const preview = n.summary.split('\n')[0].substring(0, 120);
           nc.appendChild(el('div', { class: 'tree-item-summary' }, preview));
        }

        nc.appendChild(el('div', { class: 'tree-item-meta' },
          el('span', { class: 'badge badge-default' }, n.node_type || ''),
          el('span', { class: 'badge badge-default' }, `L${n.start_line}–${n.end_line}`),
        ));
        ni.appendChild(nc);
        nodeItem.appendChild(ni);
        
        if (isNodeExpanded && n.summary) {
          const detail = el('div', { class: 'summary-detail', style: { paddingLeft: '32px' } });
          const textContainer = el('div', { class: 'summary-detail-text', style: { whiteSpace: 'pre-wrap', lineHeight: '1.6', marginBottom: '8px' } });
          textContainer.textContent = n.summary;
          detail.appendChild(textContainer);
          
          const detailActions = el('div', { class: 'summary-detail-actions' });
          detailActions.appendChild(el('button', { class: 'btn btn-ghost', onClick: (e) => { e.stopPropagation(); copyText(n.summary); }, html: Icons.copy + '<span>Copy</span>' }));
          detail.appendChild(detailActions);
          
          nodeItem.appendChild(detail);
        }
        
        frag.appendChild(nodeItem);
      }
    }
  }
...
function renderSearch() {
  ...
  input.addEventListener('input', () => {
    ...
    const filtered = defaults.filter(r => r.text.toLowerCase().includes(q) || r.hint.toLowerCase().includes(q));

    // Also search nodes
    if (state.nodes?.nodes) {
      for (const n of state.nodes.nodes) {
...
// LEFT PANE (Tree)
const leftPane = el('div', { class: 'graph-left-pane', style: { width: '260px', borderRight: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', background: 'var(--bg-panel)', overflowY: 'auto' } });
...
// CENTER PANE (Graph)
const centerPane = el('div', { class: 'graph-center-pane', style: { flex: 1, position: 'relative', background: 'var(--bg-default)' } });
...
const rightPane = el('div', { id: 'inspector-container', class: 'graph-right-pane', style: { width: '300px', borderLeft: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', background: 'var(--bg-panel)', padding: '16px', overflowY: 'auto' } });
...
function updateInspector() {
    ...
    if (state.structure) {
        target = findInTree(state.structure);
        if (target) isFile = (target.type === 'file');
    }
    
    // Try finding in nodes if not found
    if (!target && state.nodes && state.nodes.nodes) {
        target = state.nodes.nodes.find(n => n.id === state.selectedId);
    }
...
const nodeData = { id: n.id, title: n.title || n.node_type || 'Unknown', group: n.node_type || 'node', radius: 8 };
...
state.nodes.nodes.forEach(n => {
if (n.parent_id && nodesMap.has(n.parent_id)) {
links.push({
source: n.parent_id,
target: n.id,
value: 2
